...
function getaddDivarea(divid,colnms,colmdsJson,foridsel,seldivid,bn_div){
	$("#"+divid).html("");
	var _arr = [];
	_arr.push("<table id='addUser' class='ui-form-table'  border='0' style='width: 100%;'>");
	for(var i=0;i<colmdsJson.length;i++){
		//类型为文本域的为特殊情况  需要独占一行
		if(colmdsJson[i].type==5 && i>0 && i%2!=0){
			_arr.push("<td></td></tr><tr>");
			...
			_arr.push(rtStr);
			_arr.push("</tr>");
		}else if(colmdsJson[i].type==5 && i>0 && i%2==0){
			_arr.push("<tr>");
			var rtStr=elementLayout(colmdsJson[i].type,colmdsJson[i].name,colnms[i],1,colmdsJson[i].selid,foridsel,colmdsJson[i].check,seldivid,bn_div);
			_arr.push(rtStr);
			_arr.push("</tr>");
			...
			if(colmdsJson[i].name!="id"){
				...
				_arr.push('<tr><input class="add_input"  type="hidden" value="" id="'+colmdsJson[i].name+'"></tr>');
			}
			if(i!=0 && i%2==0){
				_arr.push("</tr>");
			}
...
function elementLayout(cltype,clcode,clname,lr,selid,foridsel,cktype,seldivid,bn_div){
	...
	if(cltype==1){
		return '<td class="add_lfspan"><span id="span_'+clcode+'">'+clname+'</span></td><td class="add_rtspan"><input class="add_input" type="text" value="" id="'+clcode+'">'+ckStr+'</td>';
	}else if(cltype==2){
		return '<td class="add_lfspan"><span class="" id="span_'+clcode+'">'+clname+'</span></td><td class="add_rtspan"><input class="add_input" type="hidden" id="'+clcode+'"><input name="'+selid+'" class="add_input_dis" type="text" readonly="true" id="dis_'+clcode+'">&nbsp;<span id="sp_rt_'+clcode+'" class="add_fwid" onclick="forSelId(\''+foridsel+'\','+selid+',\''+seldivid+'\',\''+clname+'\',\''+clcode+'\',\''+bn_div+'\')">...</span>'+ckStr+'</td>';
	}else if(cltype==3){
		return '<td class="add_lfspan"><span class="" id="span_'+clcode+'">'+clname+'</span></td><td class="add_rtspan"><select class="add_select" id="'+clcode+'" name='+selid+'><option value="0">请选择</option></select>'+ckStr+'</td>';
	}else if(cltype==4){
		return '<td class="add_lfspan"><span class="" id="span_'+clcode+'">'+clname+'</span></td><td class="add_rtspan"><input class="add_check" type="checkbox" id="'+clcode+'">'+ckStr+'</td>';
	}else if(cltype==5){
		return '<td class="add_lfspan"><span class="" id="span_'+clcode+'">'+clname+'</span></td><td class="add_rtspan_texa" colspan="3"><textarea class="add_texa" value="" id="'+clcode+'"></textarea>'+ckStr+'</td><td></td><td></td>';
	}
...
function forSelId(_mt,selid,seldivid,clname,clcode,bn_div){
	var gdid = seldivid;
	s=$("<DIV ID=\"form_selid_"+gdid+"\" STYLE=\"overflow:auto;position:relative;left:0px;top:0px;\" CLASS=\"jqForm\" TITLE=\"弹出选择\">" +
