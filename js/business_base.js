...
/*
 * @通用弹出选择页面调用函数 @seltype 选择类型，用于确定选择的html @seltitle 选择标题，弹出选择标题 @rtnvalobj
 * 返回值对应列 @rtndisobj 返回显示值对应列 @discol 显示取值列
 */
function comsel(seltype, seltitle, rtnvalobj, rtndisobj, discol) {
	var url = "";
	if (seltype == "selnode") {
		url = "../comsel/selnode.html";
	} else if (seltype == "selcompany") {
		url = "../comsel/selcompany.html";
	} else if (seltype == "selcity") {
		url = "../comsel/selcity.html";
	} else if (seltype == "seluser") {
		url = "../comsel/binduser.html";
	} else if (seltype == "selperiod") {
		url = "../comsel/selperiod.html";
	} else if (seltype == "selorg") {
		url = "../comsel/selorg.html";
	} else if (seltype == "selrole") {
		url = "../comsel/selrole.html";
	}

	var s = $("<DIV ID=\"com_sel_" + seltype + "\"></DIV>");
