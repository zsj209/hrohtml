$(function() {
	...
	var clmrequestData = {
		_mt : "contacts.getContactsByPage"	
	};// 获取数据的接口
	var gdid = "jq_comp_contacts";// 显示数据的div
	var addgdid = "form_jq_comp_contacts";// 新增 修改 显示数据的div
	// var addtb_div="pays_tax_addinfoarea";//新增 修改 显示字段的div
	var seldiv = "comp_contacts";// 根据id查询的divid
	var updinterface = "contacts.selectById";// 修改时根据id获取数据的接口
	var upinfointerface = "contacts.addorupdate";// 修改数据的接口
	var ininfointerface = "contacts.addorupdate";// 新增数据的接口
	var dlinfointerface = "contacts.deleteBatchById";// 批量删除接口
	var bn_div = "bn_comp_contacts";// 按钮div的id
	...
	function gettableHead() {
		var data = [];
		data.push("id,联系人姓名,职位,电话,电子邮件,地区名称,地址,上级领导,备注");
		data.push("[{name:'id'}"
				+ ",{name:'contName',width:'100'}"
				+ ",{name:'contPosition',width:'100'}"
				+ ",{name:'contIphone',width:'100'}"
				+ ",{name:'contEmail',width:'100'}"
				+ ",{name:'contDistrictName',width:'100'}"
				+ ",{name:'contAddr',width:'100'}"
				+ ",{name:'contSuperior',width:'100'}"
				+ ",{name:'contNote',width:'100'}]");
	...
	function disjQgridTable(colNames, colModel) {
		...
		$("#bn_add" + bnsuffix).bind("click", {
			...
			bn_div : bn_div
		}, addinfo_compcontacts);// 新增方法名请改成自己的
		// 此处需要修改 修改按钮
	 ...
	 * 新增方法
	 */
	function addinfo_compcontacts(e) {
...
$("#contDistrict_Name").click(function() {
	comsel("selcity", "城市", "contDistrict", "contDistrictName", 0);
})