...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		var p = $.extend(true,{
			/*自定义字段mylevel表示访问方法的安全级别:"UserLogin"加密的盐使用wtk,"None":加密使用默认的盐*/
            mylevel:"",
            /*自定义字段myDataFormat表示自定义数据格式:"myData"为3个参数调用encryptNew（）,"None"或者为空:为n个参数，调用encrypt（）*/
            myDataFormat:"",
			url: "",
			height: 150,
			page: 1,
			...
			//prmNames: {page:"page",rows:"rows",order: "sord"},
			forceFit : false,
			gridstate : "visible",
			cellEdit: false,
			cellsubmit: "remote",
		...
		populate = function (npage) {
			if(!ts.grid.hDiv.loading) {
				...
				/*TODO 添加生成_sig并添加到postData中， -->ts.p.mylevel:为自定义的安全级别*/
                ts.p.postData =  ts.p.myDataFormat === "myData" ? encryptNew(ts.p.mylevel, ts.p.postData):encrypt(ts.p.mylevel, ts.p.postData);
				...
				case "script":
					$.ajax($.extend({
						...
						/*TODO 修改响应，仅仅取出需要表格中显示的部分，待增加分页后统一返回数据的格式*/
							...
							/*修改loadComplete事件,增加allow返回值，默认为true允许生成表格，当allow为false时阻止生成表格*/
							var allow = true;
							if(lc) { allow = lc.call(ts,result); }
